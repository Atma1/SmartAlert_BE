  ...
  try {
    const [rows] = await pool.execute(
      `SELECT DATE(timestamp) AS date,
            ...
            FROM sensor_history
            WHERE timestamp >= CURDATE() - INTERVAL ? DAY
            GROUP BY date
            ORDER BY date`,
      [days]
    );
...
export const getSensorPerformance = async (req: Request, res: Response) => {
  try {
    const [rows] = await pool.execute(
      `SELECT s.id, s.name, s.location, sh.temperature, sh.moisture, sh.movement, s.status,
                CASE sh.status
        ...
        JOIN sensor_history sh ON sh.sensor_id = latest_sh.sensor_id AND sh.timestamp = latest_sh.latest`
    );
    res.json(rows);
  } catch (error) {
...
export const createSensorLog = async (req: Request, res: Response) => {
  const { sensor_id, timestamp, temperature, moisture, movement } = req.body;
  console.log("request body:", req.body);
  if (sensor_id == null || timestamp == null || temperature === undefined || moisture === undefined || movement === undefined) {
  ...
  try {
    ...
    await pool.execute(
      `INSERT INTO sensor_history (sensor_id, timestamp, temperature, moisture, movement, status)
         VALUES (?, ?, ?, ?, ?, ?)`,
...
export const exportSensorTrendCsv = async (req: Request, res: Response) => {
  ...
  const days = validRanges[range as keyof typeof validRanges];
  if (!days) {
